#!/usr/bin/env node
/**
 * Embed shared/figma-dtcg-mapping into figma-token-plugin/code.js.
 *
 * Usage:
...
function main() {
  ...
  if (checkOnly) {
    if (existing === generated) {
      console.log('figma-token-plugin mapping is in sync with shared/figma-dtcg-mapping')
      process.exit(0)
    }
    console.error(
  ...
  if (existing) {
    ...
    // Use slice splice — String.replace treats `$` specially in replacements.
    pluginSource =
  ...
  // Verify immediately so drift cannot go unnoticed.
  ...
  console.log('Embedded shared/figma-dtcg-mapping into figma-token-plugin/code.js')
}
